...
namespace {
	...
	{
		// Max is there to avoid giving health point in case too high defence.(max est ici pour éviter de donner des points de santé en cas de defense trop élevée)
...
void north()
{
	...
	// Get the location at north of the current player.(obtenez l' emplacement au nord du joueur actuel)
	TileType tile_type = get_tile_at_position(player.x, player.y - 1);
	// If the location is not empty do NOTHING! (si la location n est pas vide ne rien faire)
...
void south()
{
	Player player = get_player();// complete me!
	TileType tile_type = get_tile_at_position(player.x, player.y + 1);
	if (tile_type != TileType::EMPTY)
		return;
	player.y += 1;
